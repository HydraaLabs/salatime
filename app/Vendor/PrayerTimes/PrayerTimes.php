...
namespace App\Vendor\PrayerTimes;

use DateTime;
use DateTimezone;
use App\Vendor\MoonSighting\Fajr;
use App\Vendor\MoonSighting\Isha;
...
class PrayerTimes
{
     ...
     */
    public function __construct(string $method = Method::METHOD_MWL, string $school = self::SCHOOL_STANDARD, $asrShadowFactor = null)
    {
        $this->loadMethods();
...
//    }
    ...
    public function getTimesForToday($prayerDate, $latitude, $longitude, $timezone, $elevation = null, $latitudeAdjustmentMethod = self::LATITUDE_ADJUSTMENT_METHOD_ANGLE, $midnightMode = null, $format = self::TIME_FORMAT_24H)
    {
        $date = new DateTime($prayerDate, new DateTimezone($timezone));

        $times = $this->getTimes($date, $latitude, $longitude, $elevation, $latitudeAdjustmentMethod, $midnightMode, $format);

        // Always add Ramadan times
        $times['Sehri'] = $times[self::FAJR];      // Sehri ends at Fajr
        $times['Iftar'] = $times[self::MAGHRIB];   // Iftar time at Maghrib
    ...
    /**
     * Get Ramadan Iftar and Sehri times for a specific date
     *
     * @param DateTime $date
     ...
     * @param string $latitudeAdjustmentMethod
     * @param string|null $midnightMode
     * @param string $format
     * @return array ['iftar' => string, 'sehri' => string, 'imsak' => string]
     */
    ...
    {
        // Get all prayer times for the date
        $times = $this->getTimes(
            $date,
            $latitude,
            $longitude,
        ...
        return [
            'sehri' => $times[self::FAJR],      // Sehri ends at Fajr
            'imsak' => $times[self::IMSAK],     // Stop eating time (precautionary)
            'iftar' => $times[self::MAGHRIB],   // Iftar time at Maghrib
        ];
    }

    /**
     * Get Ramadan Iftar and Sehri times for today based on user's location
     *
     * @param float $latitude
     * @param float $longitude
     ...
     * @throws \Exception
     */
    public function getRamadanTimesForToday(
        $latitude,
        $longitude,
        $timezone,
        $elevation = null,
