<template>
    ...
    <div v-else class="lp-wrapper">
        <div class="lp-header">
            ...
            <div>
                <h2 class="lp-title">{{ $t('landing_footer.title') }}</h2>
                <p class="lp-subtitle">{{ $t('landing_footer.subtitle') }}</p>
            </div>
            <a href="/" target="_blank" class="preview-btn">{{ $t('landing_footer.preview_page') }} ↗</a>
        </div>

        <form @submit.prevent="submit">
            <div class="lp-card">
                <div class="card-label"><span class="dot"></span> {{ $t('landing_footer.section.brand') }}</div>
                <div class="fields-grid">
                    <div class="field-group">
                        <label class="field-label">{{ $t('landing_footer.form.app_name_label') }} <span class="hint">({{ $t('landing_footer.form.app_name_hint') }})</span></label>
                        <input class="field-input" type="text" :placeholder="$t('landing_footer.form.app_name_placeholder')" v-model="form.app_name" />
                        <small class="field-error" v-if="errors.app_name">{{ errors.app_name[0] }}</small>
                    </div>
                    <div class="field-group full">
                        <label class="field-label">{{ $t('landing_footer.form.footer_description_label') }}</label>
                        <textarea class="field-input field-textarea" rows="4" :placeholder="$t('landing_footer.form.footer_description_placeholder')" v-model="form.footer_description"></textarea>
            ...
            <div class="lp-card">
                <div class="card-label"><span class="dot"></span> {{ $t('landing_footer.section.footer_preview') }}</div>
                <div class="footer-preview">
                    <div class="fp-brand">
                        <span class="fp-icon">🕌</span>
                        <span class="fp-name">{{ form.app_name || 'SalaTime' }}</span>
                    </div>
                    <p class="fp-desc">{{ form.footer_description || $t('landing_footer.preview.default_description') }}</p>
                    <p class="fp-copy">© {{ new Date().getFullYear() }} {{ form.app_name || 'SalaTime' }} {{ $t('landing_footer.preview.copyright_suffix') }}</p>
                </div>
            </div>
...
const errors = ref({});
const form = ref({ app_name: '', footer_description: '' });
...
const footerFields = ['app_name', 'footer_description'];

const submit = () => {
    saving.value = true;
    errors.value = {};
    const payload = Object.fromEntries(footerFields.map(k => [k, form.value[k]]));
    Axios.post('landing-settings', payload)
        .then(({ data }) => toast.success(data.message))
        .catch(({ response }) => {
            if (response?.status === 422) errors.value = response.data.errors;
            else toast.error(response?.data?.message ?? t('landing_footer.error_saving'));
        })
...
.footer-preview { background: #111827; border-radius: 10px; padding: 20px 22px; }
.fp-brand { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
.fp-icon { font-size: 1.2rem; }
.fp-name { color: #fff; font-weight: 700; font-size: 1.1rem; }
.fp-desc { color: rgba(255,255,255,0.45); font-size: .84rem; line-height: 1.5; margin: 0 0 12px; max-width: 280px; }
.fp-copy { color: rgba(255,255,255,0.25); font-size: .78rem; margin: 0; }
