    ...
    <div class="acp-page">
        ...
        <div class="acp-card">
            ...
            <div class="acp-card__header">
                ...
                <div class="acp-card__icon-wrap">
                    <svg width="28" height="28" viewBox="0 0 32 32" fill="none">
                        <circle cx="16" cy="16" r="11" stroke="rgba(255,255,255,0.9)" stroke-width="1.8"/>
                        <path d="M11 16l4 4 6-7" stroke="rgba(212,168,67,0.9)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>
                <h2 class="acp-card__title">Set new password</h2>
                <p class="acp-card__sub">Choose a strong password for your account</p>
            </div>
            ...
            <div class="acp-card__body">
                ...
                <!-- New password -->
                <div class="af-field">
                    <label class="af-label">New password <span class="af-req">*</span></label>
...
const props = defineProps({ token: { required: true } });
const errors = ref({});
...
const submit = () => {
    ...
    errors.value = {};
    const parsed = JSON.parse(props.token);
    formData.value.token = parsed.token;
    formData.value.email = parsed.email;
    Axios.post('password/reset', formData.value)
        .then(response => { toast.success(response.data.message); window.location.href = '/dashboard'; })
        .catch(({ response }) => {
...
.acp-back { text-align: center; margin-top: 1.2rem; }
.acp-back__link {
    display: inline-flex; align-items: center; gap: 0.3rem;
    font-size: 0.8rem; font-weight: 600; color: #6b8c7a;
